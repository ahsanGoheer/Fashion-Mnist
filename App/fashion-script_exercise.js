...
function getModel() {
    ...
    model.add(tf.layers.maxPooling2d({poolSize:[2,2]}));
    // model.add(tf.layers.conv2d({filter:32,kernelSize:3,activation:'relu'}));
    // model.add(tf.layers.maxPooling2d({poolSize:[2,2]}));
...
async function train(model, data) {
    ...
    const container = {name:"Model",styles:{height:'640px'}};
    ...
    const [trainXs, trainYs] = tf.tidy(()=>{
        const fashion_data= data.nextTrainBatch(TRAIN_DATA_SIZE);
        return [
            fashion_data.xs.reshape([TRAIN_DATA_SIZE,28,28,1]),
            fashion_data.labels
    ...
    const [testXs, testYs] = tf.tidy(()=>{
        const fashion_data=data.nextTestBatch(TEST_DATA_SIZE);
        return[
            fashion_data.xs.reshape([TEST_DATA_SIZE,28,28,1]),
            fashion_data.labels
        ];
    });
